                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    auto linearSolver = std::unique_ptr<g2o::BlockSolver_6_3::LinearSolverType>(new g2o::LinearSolverEigen<g2o::BlockSolver_6_3::PoseMatrixType>());
...
void Optimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
{
    ...
    auto linearSolver = std::unique_ptr<g2o::BlockSolverX::LinearSolverType>(new g2o::LinearSolverEigen<g2o::BlockSolverX::PoseMatrixType>());

    auto solver_ptr = std::unique_ptr<g2o::BlockSolverX>(new g2o::BlockSolverX(std::move(linearSolver)));
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    auto linearSolver = std::unique_ptr<g2o::BlockSolver_6_3::LinearSolverType>(new g2o::LinearSolverDense<g2o::BlockSolver_6_3::PoseMatrixType>());
...
{   
    ...
    g2o::SparseOptimizer optimizer;
    optimizer.setVerbose(false);
    auto linearSolver = std::unique_ptr<g2o::BlockSolver_7_3::LinearSolverType>(new g2o::LinearSolverEigen<g2o::BlockSolver_7_3::PoseMatrixType>());
    auto solver_ptr= std::unique_ptr<g2o::BlockSolver_7_3>(new g2o::BlockSolver_7_3(std::move(linearSolver)));
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(std::move(solver_ptr));
...
{
    g2o::SparseOptimizer optimizer;
    auto linearSolver = std::unique_ptr<g2o::BlockSolverX::LinearSolverType>(new g2o::LinearSolverDense<g2o::BlockSolverX::PoseMatrixType>());
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA)
{
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(pKFi->mPrevKF && pKFi->mnId<=maxKFid)
        {
            ...
            g2o::HyperGraph::Vertex* VGt = optimizer.vertex(maxKFid*2+2);
            g2o::HyperGraph::Vertex* VAt = optimizer.vertex(maxKFid*2+3);
            g2o::HyperGraph::Vertex* VGDirt = optimizer.vertex(maxKFid*2+4);
            g2o::HyperGraph::Vertex* VSt = optimizer.vertex(maxKFid*2+5);
            if(!VP1 || !VV1 || !VGt || !VAt || !VP2 || !VV2 || !VGDirt || !VSt)
            {
                std::cout << "Error" << VP1 << ", "<< VV1 << ", "<< VGt << ", "<< VAt << ", " << VP2 << ", " << VV2 <<  ", "<< VGDirt << ", "<< VSt <<std::endl;
            ...
            ei->setVertex(0,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VP1));
            ei->setVertex(1,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VV1));
            ei->setVertex(2,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VGt));
            ei->setVertex(3,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VAt));
            ei->setVertex(4,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VP2));
            ei->setVertex(5,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VV2));
            ei->setVertex(6,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VGDirt));
            ei->setVertex(7,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VSt));
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale)
{
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(pKFi->mPrevKF && pKFi->mnId<=maxKFid)
        {
            ...
            g2o::HyperGraph::Vertex* VGDirt = optimizer.vertex(4*(maxKFid+1));
            g2o::HyperGraph::Vertex* VSt = optimizer.vertex(4*(maxKFid+1)+1);
            if(!VP1 || !VV1 || !VG || !VA || !VP2 || !VV2 || !VGDirt || !VSt)
            {
                Verbose::PrintMess("Error" + std::to_string(VP1->id()) + ", " + std::to_string(VV1->id()) + ", " + std::to_string(VG->id()) + ", " + std::to_string(VA->id()) + ", " + std::to_string(VP2->id()) + ", " + std::to_string(VV2->id()) +  ", " + std::to_string(VGDirt->id()) + ", " + std::to_string(VSt->id()), Verbose::VERBOSITY_NORMAL);
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF, bool *pbStopFlag, Map *pMap, LoopClosing::KeyFrameAndPose &corrPoses)
{
    ...
    // Check inlier observations
    // Mono
    for(size_t si=0, iend=vpEdgesMono.size(); si<iend;si++)
    {
        EdgeMono* e = vpEdgesMono[si];
        MapPoint* pMP = vpMapPointEdgeMono[si];
        ...
        if(e->chi2()>chi2Mono2)
        {
            KeyFrame* pKFi = vpEdgeKFMono[si];
            vToErase.push_back(std::make_pair(pKFi,pMP));
        }
    }

    // Stereo
    for(size_t si=0, iend=vpEdgesStereo.size(); si<iend;si++)
    {
        EdgeStereo* e = vpEdgesStereo[si];
        MapPoint* pMP = vpMapPointEdgeStereo[si];
    ...
    if(!vToErase.empty())
    {
        for(size_t si=0;si<vToErase.size();si++)
        {
            KeyFrame* pKFi = vToErase[si].first;
            MapPoint* pMPi = vToErase[si].second;
            pKFi->EraseMapPointMatch(pMPi);
            pMPi->EraseObservation(pKFi);
        }
    }
