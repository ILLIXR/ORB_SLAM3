    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    else
    {
        ...
        //cout << "KF in DB: " << mpKeyFrameDatabase->mnNumKFs << "; words: " << mpKeyFrameDatabase->mnNumWords << std::endl;
        ...
        //cout << "Binary file read in " << msElapsed << " ms" << std::endl;
...
void System::SaveTrajectoryEuRoC(const std::string &filename)
{
    ...
    //cout << "size mlpReferences: " << mpTracker->mlpReferences.size() << std::endl;
    //cout << "size mlRelativeFramePoses: " << mpTracker->mlRelativeFramePoses.size() << std::endl;
    //cout << "size mpTracker->mlFrameTimes: " << mpTracker->mlFrameTimes.size() << std::endl;
    //cout << "size mpTracker->mlbLost: " << mpTracker->mlbLost.size() << std::endl;
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        while(pKF->isBad())
        {
            ...
            Trw = Trw * pKF->mTcp;
            pKF = pKF->GetParent();
            //cout << "--Parent KF: " << pKF->mnId << std::endl;
        }

        if(!pKF || pKF->GetMap() != pBiggerMap)
        {
            //cout << "--Parent KF is from another map" << std::endl;
            continue;
        }
