...
public:
        ...
        , root_path{getenv("ORB_SLAM_ROOT")}
    {
	    ...
	    boost::filesystem::path setting_path = "/home/illixr/henry/ORB_SLAM3/Examples/RGB-D/ASUS.yaml";
...
#warning "No OpenCV metrics available. Please recompile OpenCV from git clone --branch 3.4.6-instrumented https://github.com/ILLIXR/opencv/. (see install_deps.sh)"
        ...
        std::cout << std::fixed << std::setprecision(6) << duration2double(datum->time.time_since_epoch()) << setprecision(9) 
                                                    << " " << trans[0] 
                                                    << " " << trans[1] 
                                                    << " " << trans[2] 
                                                    << " " << quat.x() 
